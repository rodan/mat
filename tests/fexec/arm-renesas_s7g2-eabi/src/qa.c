...
//mmap() seems to not be implemented, so we skip the scenarios that depend on it
...
// the f_int() function below, in machine code
#define OPCODE_LEN 6
uint8_t f_opcode[OPCODE_LEN] = { 0x00, 0xbf, 0xee, 0x20, 0x70, 0x47 };

uint8_t f_int(void);
void display_menu(void);
int qa_run_test(void);
...
void display_menu(void)
{
    uart0_print("\r\n fexec test --- general commands:\r\n\r\n");
    uart0_print(" \e[33;1m?\e[0m             - show menu\r\n");
    uart0_print(" \e[33;1mr\e[0m             - run test\r\n");
}
...
int qa_run_test(void)
{
...
#if 0
    // disabled due to the fact that compiler optimizations remove the nop opcode from f_int in some cases
    uint8_t i;
    uint8_t *ap;
    ap = (uint8_t *)&f_int;
    ...
    if (memcmp(current_opcode, f_opcode, OPCODE_LEN) != 0) {
        ...
        for (i=0; i<OPCODE_LEN; i++) {
            //printf of ("0x%02x", 0) gets optimized out?! @#%% off
            //printf("0x%02x%s", (uint8_t)(*(ap+i)), i < (OPCODE_LEN-1) ? ", ":" ");
            printf("0x%02x", (uint8_t)(*(ap+i)));
            if (i < (OPCODE_LEN-1)) {
                printf(", ");
            }
            //printf("<-%d ",i);
...
#ifdef SCENARIO5
    printf("* execute stored opcodes without mmap, also not obeying NX %p", &f_opcode);
    // place a breakpoint on the next line and step into it - the code will magically work (and not crash)
